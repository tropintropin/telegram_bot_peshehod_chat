...
from keyboards.inline_keyboards import (
    create_faq_section_item_inline_kb,
...
async def process_faq_specs_item_press(
    callback: CallbackQuery, callback_data: ItemsFAQCallbackFactory
):
    if callback_data.item == "tours":  # Move to Tours Section if true
        ...
        if callback.message:
            ...
            items_list_keyboard = create_faq_section_item_inline_kb(
                1, items_list, callback_data.section
            )
            await callback.message.answer(
                text=rf"<strong>{list_name}</strong>", reply_markup=items_list_keyboard
            )

            item_specs = items_list[callback_data.item]
            text = f"""<strong>{item_specs['question']}</strong>
...
async def process_faq_specs_press(
    callback: CallbackQuery, callback_data: FAQCallbackFactory
):
    if callback.message:
        sections = get_faq_sections()
        faq_section_keyboard = create_faq_section_list_inline_kb(
            width=1, user_dict=sections
        )
        ...
        items_list_keyboard = create_faq_section_item_inline_kb(
            width=1, user_dict=items_list, section=callback_data.section
        )
...
async def process_faq_press(callback: CallbackQuery):
    ...
    This function processes the callback query when
    the "FAQ" button is pressed.
